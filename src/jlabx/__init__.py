"""jlabx — Launch JupyterLab with extensions."""
...
def _config_dir() -> Path:
    ...
    if xdg:
        return Path(xdg) / "jlabx"
    return Path.home() / ".config" / "jlabx"
...
def _cmd_add(packages: list[str]) -> None:
    if not packages:
        print("Usage: jlabx add <package> [package...]")
        sys.exit(1)
...
def _cmd_remove(packages: list[str]) -> None:
    if not packages:
        print("Usage: jlabx remove <package> [package...]")
        sys.exit(1)
...
def _cmd_help() -> None:
    print("jlabx — Launch JupyterLab with extensions")
    print()
    print("Usage:")
    ...
    print("  jlabx remove <pkg> [pkg...]  Remove user extensions")
    print()
    print(f"Config: {_config_file()}")
    print()
    print("Install: uv tool install jlabx")
